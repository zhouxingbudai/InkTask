...
| 产物 | 大小 | 适用场景 |
| --- | --- | --- |
| `InkTask-win64-1.6.1.zip` | 151 MB | 绿色解压版，解压到任意目录即可长期使用 |
...
4. **分组管理**：**双击分组条上的芯片就地重命名**（`Enter` 保存、`Esc` 取消），或分组条末端「＋」打开管理面板——新建（回车确认）、重命名（行内笔形按钮或双击）、删除（其任务自动移入未分组，5 秒内可撤销）。旧数据升级后若出现「分组N」占位名，提示条上「去重命名」一键改回原名。
