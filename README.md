...
[![Version](https://img.shields.io/badge/version-1.4.0-ffcc33?style=flat-square)](../../releases)
...
适合「每天上班打卡签退」「每周周报」「每月对账」这类周期性事务。任务卡片带重复徽章（每天 / 工作日 / 每周 / 每月 / 每 N 天），勾选即打卡：

- 到期时间自动推进到下一次出现，任务**保持未完成**持续留在列表
- **连击计数**：连续周期打卡才累计（断档重新计数），徽章直接显示连续次数
- 漏掉的周期自动跳过——出差三天回来打卡，只推进到下一次，不会攒出一堆补打卡
- 详情区显示本周期状态 / 连击 / 上次打卡时间，打卡 5 秒内可撤销
...
| 产物 | 大小 | 适用场景 |
| --- | --- | --- |
| `InkTask-Portable-1.4.0.exe` | 99 MB | 单文件便携版，双击即用，U 盘可带走 |
| `InkTask-win64-1.4.0.zip` | 150 MB | 绿色解压版，适合放到固定目录长期使用 |
...
4. **完成与清理**：点卡片圆形勾选完成；底部「清除已完成」一键清理。重复任务点勾选即打卡，到期自动顺延到下一次。
5. **设为重复**：展开任务，点「重复」按钮选择频率（每天 / 工作日 / 每周 / 每月 / 每 N 天），适合打卡签退、周报月报。
6. **快捷呼出**：任何时候 `Ctrl+Shift+Space` 呼出或隐藏面板，`Ctrl+Shift+P` 保持置顶边看边办。
...
│       ├── editor.js     # 富文本编辑 + 贴图
│       ├── urgency.js    # 紧急度评分与排序
│       ├── recur.js      # 重复任务引擎（打卡 / 连击）
│       ├── storage.js    # 存储适配（Electron / 浏览器）
│       └── util.js
├── tests/                # node:test 单元测试（42 项）
├── docs/
│   ├── ADR.md            # 架构决策记录
│   └── images/           # 文档截图
├── build/                # 图标资源
...
- **重复任务引擎**：纯函数推进 `dueAt`（锚点 + 周期跳过 + 月末钳制），打卡不生成新任务、不污染完成列表，见 `renderer/js/recur.js`。
