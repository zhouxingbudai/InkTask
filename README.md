...
4. **完成与清理**：点卡片圆形勾选完成；底部「清除已完成」一键清理——重复任务默认保留并询问去留（提示条上点「一并清除」可连它们一起删，误删可撤销）。
5. **设为重复**：创建时在快速新增栏点「重复」，或展开任务点「重复」按钮选择频率（每天 / 工作日 / 每周 / 每月 / 每 N 天）。完成后进入已完成区，到下一周期所在日 0 点自动回到待办，适合下班签退、周报月报。
