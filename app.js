...
(function () {
  ...
  /**
   * 孤儿分组恢复：任务引用的 groupId 在 groups 里找不到定义时，按引用各建一个占位分组。
   * 历史版本存在「保存时丢弃 groups 字段」的缺陷，分组定义从未落盘，但任务上的 groupId
   * 引用还在——重建占位分组后任务仍正确归堆，用户把占位名改成原分组名即完成恢复。
   */
  function recoverOrphanGroups() {
    const gs = groups();
    const known = new Set(gs.map((g) => g.id));
    const orphans = new Map(); // groupId -> 引用任务数
    tasks().forEach((t) => {
      if (t.groupId && !known.has(t.groupId)) orphans.set(t.groupId, (orphans.get(t.groupId) || 0) + 1);
    });
    ...
    for (const id of orphans.keys()) {
      gs.push({
        id,
        name: `分组${gs.length + 1}`,
        color: GROUP_PALETTE[gs.length % GROUP_PALETTE.length],
        createdAt: Date.now()
      });
    }
    persist();
...
<article class="task done" data-id="${t.id}">
  ...
  async function reloadFromMain() {
    ...
    recoverOrphanGroups(); // 切到旧版数据目录时同样可能遇到孤儿分组引用
  ...
  function bindSettings() {
    ...
    $('#btn-import').addEventListener('click', async () => {
      await Storage.flush(); // 先落盘当前编辑，导入合并需基于最新本地数据
  ...
  function listenMain() {
    ...
    // 到期提醒标记补丁：只原位改 notifiedDue 标志，绝不整体替换文档，
    // 从根上杜绝主进程到期检查把防抖窗口内未落盘的编辑冲掉
    window.inktask.on('tasks-patch', (patch) => {
      if (!patch || !Array.isArray(patch.notifiedDue)) return;
      for (const p of patch.notifiedDue) {
        if (!p || !p.id) continue;
        const t = findTask(p.id);
        if (t) t.notifiedDue = !!p.notifiedDue;
      }
    });
  ...
  function adoptExternal(doc) {
    ...
    // 版本守卫：渲染层保存有 400ms 防抖窗口，主进程广播可能携带落后于本地
    // 未落盘编辑的旧文档。updatedAt 不占优时忽略，防止旧数据整体回写，
    // 冲掉刚做的分组/勾选/改名等编辑。
    const ext = Number(doc.meta && doc.meta.updatedAt) || 0;
    const cur = Number(state.doc.meta && state.doc.meta.updatedAt) || 0;
    if (ext <= cur) return;
    state.doc = doc;
    recoverOrphanGroups(); // 兼容旧备份：任务引用的分组定义缺失时按引用重建
    renderAll();
  }
  ...
  async function init() {
    state.doc = await Storage.getDoc();
    recoverOrphanGroups(); // 旧版数据：任务引用的分组定义从未落盘，按引用重建占位分组
